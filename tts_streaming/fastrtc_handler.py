...
class FastRTCTTSHandler(AsyncStreamHandler):
    ...
    async def start_up(self):
        ...
        if not self.tts_queue:
            logger.warning("⚠️ TTS queue not injected - audio will not play")
        else:
    ...
    async def emit(self) -> Tuple[int, np.ndarray]:
        ...
        if self._audio_output_queue.empty() and self._current_audio_chunk is None:
            ...
            return (self._sample_rate, np.zeros((1, int(self._sample_rate * 0.02)), dtype=np.int16))
        ...
        if self._current_audio_chunk is not None:
            ...
            if remaining > 0:
                ...
                # Reshape for FastRTC (needs 2D: (channels, samples))
                if chunk.ndim == 1:
                    chunk = chunk.reshape(1, -1)
                
                return (sample_rate, chunk.astype(np.int16))
            else:
        ...
        await asyncio.sleep(0.02)
        return (self._sample_rate, np.zeros((1, chunk_size), dtype=np.int16))
    ...
    async def add_audio_chunk(self, audio_bytes: bytes, sample_rate: int):
        ...
        try:
            ...
            # Add to queue
            await self._audio_output_queue.put((audio_array, sample_rate))
            
            logger.debug(f"Added audio chunk to FastRTC queue: {len(audio_array)} samples @ {sample_rate}Hz")
