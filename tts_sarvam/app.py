...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.info(f"📋 Configuration loaded | Model: {config.sarvam_model} | Speaker: {config.sarvam_speaker}")
...
async def synthesize_text(request: SynthesizeRequest):
    ...
    if not provider:
        raise HTTPException(
            status_code=503,
            detail="Sarvam provider not initialized. Check SARVAM_API_KEY."
        )
            ...
            try:
                ...
                if cache:
                    cached_path = cache.get_cached_audio(
                        sentence,
                        request.voice or config.sarvam_speaker,
                        request.language or config.sarvam_language,
                        "sarvam",
                ...
                if cached_path:
                    ...
                    # Synthesize with Sarvam (no emotion parameter)
