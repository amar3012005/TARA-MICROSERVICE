...
class VADManager:
    ...
    def __init__(self, config: VADConfig, redis_client: Optional[Any] = None, rag_client: Optional[RAGClient] = None):
        """
        ...
        Args:
            ...
            redis_client: Optional Redis client for state persistence
            rag_client: Optional RAG client for knowledge base queries
        """
        self.config = config
        ...
        logger.info(f" VADManager initialized (model={config.model_name}, timeout={config.initial_timeout_s}s, RAG={'enabled' if rag_client else 'disabled'})")
    ...
    async def _send_to_rag_incremental(self, session_id: str, text: str, is_final: bool = False):
        """
        Helper method to send incremental queries to RAG (fire-and-forget).
        ...
        try:
            # Consume the async iterator (fire-and-forget)
            async for _ in self.rag_client.send_incremental(session_id, text, is_final):
                pass  # Just consume to trigger the request
        except Exception as e:
            logger.warning(f"⚠️ RAG incremental send error: {e}")
        ...
        if session_key not in self._active_sessions:
            ...
            try:
                ...
                receive_task = asyncio.create_task(
                    self._receive_loop(session_id, session, streaming_callback, audio_output_callback)
                )
    ...
    async def _receive_loop(
        ...
        Handles:
        - Input transcription (user speech) -> sends to RAG incrementally
        - Tool calls (query_knowledge_base) -> calls RAG and returns result
        - Audio output (model turn with inline_data) -> sends to audio callback
        - Text output (model turn with text) -> logs for debugging
        """
        logger.info(f"👂 [{session_id}] Starting background receive loop (RAG: {'enabled' if self.rag_client else 'disabled'})")
        ...
        try:
            async for response in session.receive():
                ...
                if response.server_content:
                    ...
                    # 1. Handle Input Transcription (User Speech) -> Send to RAG incrementally
                    if response.server_content.input_transcription:
                        ...
                        if final_text:
                            ...
                            if complete_text:
                                ...
                                if streaming_callback:
                                    try:
                                        streaming_callback(normalized_text, False)  # Partial
                                    except Exception as e:
                                        logger.error(f"Streaming callback error: {e}")

                                logger.info("=" * 70)
                                logger.info(f"📝 [PARTIAL] STT Fragment")
                                logger.info(f"   Text: '{normalized_text[:150]}'")
                                logger.info(f"   Session: {session_id}")
                                logger.info("=" * 70)
                                
                                # Send to RAG incrementally (non-blocking)
                                if self.rag_client:
                                    try:
                                        # Fire-and-forget incremental RAG query
                                        asyncio.create_task(
                                            self._send_to_rag_incremental(session_id, normalized_text, is_final=False)
                                        )
                                    except Exception as e:
                                        logger.warning(f"⚠️ Failed to send to RAG: {e}")
                                
                                # Publish partial transcript to Redis
                    ...
                    # 2. Handle Model Turn (Gemini Response)
                    if response.server_content.model_turn:
                        for part in response.server_content.model_turn.parts:
                            
                            # 2a. Handle Function Call (Tool Use)
                            if hasattr(part, 'function_call') and part.function_call:
                                func_call = part.function_call
                                func_name = func_call.name if hasattr(func_call, 'name') else None
                                
                                if func_name == "query_knowledge_base":
                                    logger.info(f"🔧 [{session_id}] Tool call: query_knowledge_base")
                                    
                                    # Extract query from function call arguments
                                    query = ""
                                    if hasattr(func_call, 'args') and func_call.args:
                                        query = func_call.args.get("query", "")
                                    
                                    if query and self.rag_client:
                                        try:
                                            # Get full RAG response
                                            logger.info(f"📚 [{session_id}] Querying RAG: '{query[:50]}...'")
                                            rag_response = await self.rag_client.get_full_response(session_id, query)
                                            
                                            # Send tool response back to Gemini
                                            logger.info(f"✅ [{session_id}] RAG response received ({len(rag_response)} chars)")
                                            
                                            # Format tool response according to Gemini Live API
                                            ...
                                            logger.error(f"❌ [{session_id}] RAG tool call error: {e}")
                                            # Send error response
                                            try:
                                                error_response = types.ToolResponse(
                                                    name="query_knowledge_base",
                            ...
                            # 2b. Handle Audio Output (inline_data)
                            elif hasattr(part, 'inline_data') and part.inline_data:
                                if hasattr(part.inline_data, 'data') and part.inline_data.data:
                                    audio_bytes = part.inline_data.data
                                    mime_type = getattr(part.inline_data, 'mime_type', 'audio/pcm')
                                    
                                    logger.debug(f"🔊 [{session_id}] Audio chunk received | Size: {len(audio_bytes)} bytes | MIME: {mime_type}")
                            ...
                            # 2c. Handle Text Output (for debugging)
                            elif hasattr(part, 'text') and part.text:
                                logger.debug(f"🤖 [{session_id}] Model text: {part.text[:100]}")
                    ...
                    if response.server_content.turn_complete:
                        ...
                        if final_transcript:
                            ...
                            # Send final transcript to RAG (is_final=True)
                            if self.rag_client:
                                try:
                                    asyncio.create_task(
                                        self._send_to_rag_incremental(session_id, normalized_transcript, is_final=True)
                                    )
                                except Exception as e:
                                    logger.warning(f"⚠️ Failed to send final to RAG: {e}")
