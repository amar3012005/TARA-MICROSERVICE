...
async def lifespan(app: FastAPI):
    ...
    global vad_manager, redis_client, rag_client
    
    logger.info("=" * 70)
    logger.info("🚀 Starting Gemini Live RAG Experiment Service")
    logger.info("=" * 70)
    ...
    # rag_service_url = os.getenv("RAG_SERVICE_URL", "http://tara-task-rag:8003")
    # logger.info(f"🔗 Initializing RAG Client | Service: {rag_service_url}")
    # rag_client = RAGClient(rag_service_url=rag_service_url)
    rag_client = None  # Disabled for simple Gemini Live experiment
    logger.info("⚠️ RAG Client disabled for simple experiment")
    ...
    vad_manager = VADManager(config, None, rag_client)  # Start without Redis, with RAG client
    ...
    logger.info("=" * 70)
    logger.info("✅ Gemini Live RAG Experiment Service Ready (Redis connecting in background)")
    logger.info("=" * 70)
    ...
    # Shutdown
    logger.info("=" * 70)
    logger.info("🛑 Shutting down Gemini Live RAG Experiment service...")
    logger.info("=" * 70)
    
    # Close RAG client
    if rag_client:
        logger.info("🔌 Closing RAG client...")
    ...
    logger.info("✅ Gemini Live RAG Experiment service stopped")


# Initialize FastAPI app
app = FastAPI(
    title="Gemini Live RAG Experiment",
    description="Bidirectional conversational AI with RAG integration using Gemini Live API",
...
fastrtc_stream = Stream(
    ...
    ui_args={
        ...
        "description": "Speak into your browser microphone. Audio streams to Gemini Live for bidirectional conversation with RAG-powered domain knowledge. "
                     "Check Docker console logs for pipeline progress, speech detection, transcripts, and audio responses."
    }
)
