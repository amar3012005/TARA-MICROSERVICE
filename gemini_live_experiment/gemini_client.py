...
class GeminiLiveSession:
    ...
    async def get_session(cls, config: VADConfig, force_new: bool = False) -> Any:
        ...
        try:
            # Session configuration for bidirectional conversation with RAG integration
            session_config = {
                "response_modalities": ["AUDIO", "TEXT"],  # AUDIO for TTS, TEXT for debug
                "input_audio_transcription": {},   # Enable user speech transcription
                "system_instruction": """You are TARA, a helpful customer service assistant for T.A.S.K (Telangana Academy for Skill and Knowledge). 
You can answer questions about T.A.S.K programs, registration, eligibility, fees, placement, and more.

When users ask questions, use the query_knowledge_base tool to search the T.A.S.K knowledge base for accurate, domain-specific information.
After receiving the knowledge base response, provide a helpful answer in a friendly, conversational manner.

Respond naturally in Telugu-English mixed style (Tenglish) when appropriate, or in English as needed.
Keep responses concise and helpful."""
            }
            
            # Add tool definition for RAG knowledge base queries
            session_config["tools"] = [{
                "function_declarations": [{
                    "name": "query_knowledge_base",
                    "description": "Search the T.A.S.K knowledge base for domain-specific information about programs, registration, eligibility, fees, placement, contact information, and other T.A.S.K-related queries.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "query": {
                                "type": "string",
                                "description": "The user's question or query to search in the knowledge base"
                            }
                        },
                        "required": ["query"]
                    }
