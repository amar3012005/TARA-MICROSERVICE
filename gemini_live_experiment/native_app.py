"""
Native WebRTC STT/VAD Application
=================================
...
Architecture:
Browser ==(UDP/RTP)==> [Docker: FastRTC -> VADManager -> Gemini]
...
class NativeAudioHandler(AsyncStreamHandler):
    ...
    async def start_up(self):
        ...
        if vad_manager is None:
            ...
            vad_manager = VADManager(config, None) # No Redis needed for basic flow
            logger.info("✅ VAD Manager ready")

    async def receive(self, audio: tuple) -> None:
        """
        Direct audio path: WebRTC -> Memory -> Gemini
        """
        try:
            ...
            # Convert to 16-bit PCM bytes (16kHz required by Gemini)
...
def create_ui():
    ...
    stream = Stream(
        ...
        mode="send-receive",
        ui_args={
            "title": "Leibniz Native WebRTC (UDP)",
...
if __name__ == "__main__":
    ...
    logger.info("🚀 Starting Native WebRTC Service (UDP)")
