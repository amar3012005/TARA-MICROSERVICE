...
class FastRTCSTTHandler(AsyncStreamHandler):
    ...
    def __init__(self, vad_manager=None, redis_client=None):
        ...
        # Audio output queue for TTS from Gemini Live
        self._audio_output_queue: asyncio.Queue = asyncio.Queue(maxsize=100)
        
        logger.info("🎙️ FastRTC STT Handler initialized (with audio output support)")
    ...
    async def receive(self, audio: tuple) -> None:
        ...
        try:
            ...
            if len(self._audio_buffer) >= self._buffer_limit:
                ...
                # Send directly to VADManager (non-blocking) with audio output callback
                await self.vad_manager.process_audio_chunk_streaming(
                    ...
                    audio_output_callback=self.queue_audio
                )
    ...
    def queue_audio(self, audio_bytes: bytes):
        """
        Queue audio bytes for output (called by VADManager when Gemini generates audio).
        
        Args:
            audio_bytes: Raw audio bytes (PCM format)
        """
        try:
            # Non-blocking put - drop if queue is full (prevent blocking)
            self._audio_output_queue.put_nowait(audio_bytes)
            logger.debug(f"🔊 Audio queued for output | Size: {len(audio_bytes)} bytes")
        except asyncio.QueueFull:
            # Drop oldest chunk if queue is full (leaky bucket)
            try:
                dropped = self._audio_output_queue.get_nowait()
                self._audio_output_queue.put_nowait(audio_bytes)
                logger.debug(f"⚠️ Audio queue full - dropped oldest chunk ({len(dropped)} bytes)")
            except asyncio.QueueEmpty:
                logger.warning("⚠️ Audio queue state inconsistent")
    
    async def emit(self):
        """
        Emit method required by AsyncStreamHandler.
        Yields audio chunks from the output queue (TTS from Gemini Live).
        """
        try:
            # Try to get audio from queue (non-blocking)
            audio_bytes = self._audio_output_queue.get_nowait()
            
            # Convert bytes to numpy array (int16 PCM, 16kHz mono)
            audio_array = np.frombuffer(audio_bytes, dtype=np.int16)
            
            # Reshape if needed (ensure 1D)
            if audio_array.ndim > 1:
                audio_array = audio_array.flatten()
            
            # Convert to float32 for FastRTC (normalize to [-1, 1])
